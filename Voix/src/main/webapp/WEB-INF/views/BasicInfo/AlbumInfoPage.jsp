			...
			<div>
				<div class="row">
					<div class="card col-md-8 mb-2">
						...
						<div class="card mb-2">
						<input type = "checkbox" id="${AlbumInfoList.alcode}" value = "${AlbumInfoList.alsaleprice}">${AlbumInfoList.alsaleprice}원	
						<c:if test="$('#'${AlbumInfoList.alcode}').is(':checked')== true"><!--내일 이거 해야함 체크 표시 해야 나오게  -->
					...
					<div class="card col-md-4 mb-2 " style="display: inline-block;">
						<div>
							<button class="button mb-4 mt-4"
								onclick = "location.href='/CartPage'">장바구니 바로가기</button>
						</div>
						<div>
							<button class="button" onclick="location.href='/PayPage?alcode=${ALInfo.alcode}'">바로가기</button>
						</div>
					</div>
				</div>
			</div>
